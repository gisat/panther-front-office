...
class LargeDataLayerTile {
    ...
    draw() {
        ...
        for (let i = 0; i < this._data.length; i++) {
            ...
            ctx.arc(
                ...
                Math.ceil(Math.sqrt(location.data['PERIMETER'])), 0, 2 * Math.PI, false);
            ctx.fillStyle = 'green';
            ctx.fill();
        }
    ...
    latitudeInSector(location, sector, height) {
        ...
        var locationInArea = (location.y - 90) - sector.minLatitude;
    ...
    longitudeInSector(location, sector, width) {
        ...
        var locationInArea = (location.x - 180) - sector.minLongitude;
