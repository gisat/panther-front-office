...
import AttributeMetadataScreen
	from "../../../../components/common/backOffice/metadataScreens/AttributeMetadataScreen";

const order = [['nameDisplay', 'ascending']];

const mapStateToProps = (state, props) => {
	return {
		models: Select.attributes.getAllForActiveApp(state, order)
	}
};
...
const mapDispatchToPropsFactory = () => {
	...
	return (dispatch, props) => {
		return {
			...
			onMount: () => {
				dispatch(Action.attributes.useIndexed({application: true}, null, order, 1, 1000, componentId));
			},
			onUnmount: () => {
