...
class ScopesDescription extends React.PureComponent {
		...
		return (
			<div className="esponFuore-scopes-description">
				...
				<div className="esponFuore-scopes-description-content">
					...
					{this.renderExploreButton("msa")}
					<img className="esponFuore-doc-image" src={msaCoasts}/>

					<h2 id="mountains">ESPON GEOSPECS Mountains (MTN)</h2>
					...
					{this.renderExploreButton("mountains")}
					<img className="esponFuore-doc-image" src={mountains}/>

					<h2 id="islands">ESPON GEOSPECS Islands (ISL)</h2>
					...
					{this.renderExploreButton("islands")}
	...
	renderExploreButton(key) {
		const scope = _.find(this.props.scopes, (item) => item.data.configuration && item.data.configuration.regionType === key);

		if (scope) {
			return (
				<Button onClick={this.onScopeSelect.bind(this, scope.key)}>Explore in the app</Button>
			);
		} else {
			return null;
		}
