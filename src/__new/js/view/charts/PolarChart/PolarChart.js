define([
	...
	PolarChart.prototype.rebuild = function () {
		...
		data.series.forEach(function (serie) {
			var serieIndex = 0;
			var serieValues = serie.data.map(function(o){return o.y;});
			var serieMin = Math.min.apply(Math,serieValues);
			var serieMax = Math.max.apply(Math,serieValues);

			serie.data.forEach(function (value) {
				chartData[serieIndex] = chartData[serieIndex] || [];
				chartData[serieIndex][attributeIndex] = chartData[serieIndex][attributeIndex] || [];
		...
		RadarChart.draw(cmp.el.dom, chartData, chartConfig);
