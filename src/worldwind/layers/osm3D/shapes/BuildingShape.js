...
class BuildingShape {
     ...
     */
    setColor(configuration) {
        let numberOfThresholds = configuration.heatmap.thresholds.length;
        let heat = 0.5 / (numberOfThresholds - 2);

        if (configuration.attributes.interiorColor.red < 0.5) {
            for (let thresholdIndex = 0; thresholdIndex < numberOfThresholds - 1; thresholdIndex++) {
