...
let Logger = {
    ...
    makeMessage: function (className, functionName, message) {
        let msg = this.messageTable[message] ? this.messageTable[message] : message;
    ...
    logMessage: function (level, className, functionName, message) {
        let msg = this.makeMessage(className, functionName, message);
        this.log(level, msg);
...
let loggingLevel = 4; // log severe messages by default
