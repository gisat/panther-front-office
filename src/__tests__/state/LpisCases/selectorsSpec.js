...
describe('Lpis cases selectors', () => {
	...
	it ('should select all cases', () => {
		Selector(Select.lpisCases.getCases).expect(state).toReturn(stateClone.lpisCases.cases);
	...
	it ('should select all cases extended with changes for each case', () => {
		...
		let result = Selector(Select.lpisCases.getCasesWithChanges).execute(minimalisticState);
